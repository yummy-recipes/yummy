context('Blog', () => {
  ...
  it('renders properly', () => {
    ...
    cy.get('a[class*=card]:first').should(($card) => {
      ...
      expect($card.find('div[class*=contentSummary] p')).to.contain(
        'Salami ground',
      )
      expect($card.find('div[class*=contentSummary] p').text().length).to.eq(
        237,
      )
  ...
  it('renders the post page properly', () => {
    ...
    cy.get('div[class*=postIntro]').should(($post) => {
      expect($post.find('ol li:first')).to.contain('Strona Główna')
      expect($post.find('ol li:last')).to.contain('blog')
      ...
      expect($post.find('div[class*=postHeadline] p')).to.contain(
        'Salami ground',
      )
